...
@section('column_data')
  ...
  @if( isset($items) )
    ...
    <tr>
      ...
      <td>{{ $item->id }}</td>
      <td>
        <a href="{{ url('/report-detail?id='.$item->id) }}">
        {{ $item->subject }}
        </a>
      </td>
      <td>{{ $item->expo_yn }}</td>
...
<script>
  ...
  $(function(){
    ...
    function Search(page)
    {
      ...
      $(location).attr('href', "{{ url('/report-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/report-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/report/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
