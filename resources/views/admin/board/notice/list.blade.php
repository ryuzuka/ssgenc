{{-- 
  // 페이지명 : 뉴스&공지/영상 관리 목록
  // 설명 : 뉴스&공지/영상 관리 목록을 페이지 단위로 표시 한다.
--}}

{{-- 1) 보드 리스트형식의 템플릿 --}}
@extends('layouts.board_list', [
      'title' => '게시판 관리',
      'subject' => '뉴스&공지/영상 관리 목록',   
      'lang' => 'ko',
      'tab_no' => 1,
      'menu_no' => 2
  ])
...
@section('list_count_area')
  ...
  <div class="search-total">총 <span>{{ $notice_count }}</span>건 / 검색결과
    <span>
      @if( isset($notices) ) 
        {{ $notices->total() }}
      @else
...
@section('column_data')
  ...
  @if( isset($notices) )
    ...
    <tr>
      ...
      <td>{{ $data->id }}</td>
      <td>{{ $data->news_gubun }}</td>
      <td>
        <a href="{{ url('/notice-detail?id='.$data->id) }}">
        {{ $data->subject }}
        </a>
      </td>
      <td>{{ $data->expo_yn }}</td>
...
@section('pagenation_area')
  ...
  @include('includes.paginate', ['paginator' => $notices])
...
<script>
  ...
  $(function(){
    ...
    {
      if($("#news_gubun").val() == '00')
      {
        Vue.alert('뉴스 구분을 선택해 주세요.');
        return;
      }
      ...
      $(location).attr('href', "{{ url('/notice-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/notice-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/notice/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
