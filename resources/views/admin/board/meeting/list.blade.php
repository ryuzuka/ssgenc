{{-- 
  // 페이지명 : 이사회 진행사항 관리 목록
  // 설명 : 이사회 진행사항 관리 목록을 페이지 단위로 표시 한다.
--}}
...
@section('column_data')
  ...
  @if( isset($items) )
    ...
    <tr>
      ...
      <td>{{ $item->hold_at }}</td>
      <td>
        <a href="{{ url('/meeting-detail?id='.$item->id) }}">
        {{ $item->subject }}
        </a>
      </td>
      <td>{{ $item->vote_st }}</td>
...
<script>
  ...
  $(function(){
    ...
    function Search(page)
    {
      ...
      $(location).attr('href', "{{ url('/meeting-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/meeting-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/meeting/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
