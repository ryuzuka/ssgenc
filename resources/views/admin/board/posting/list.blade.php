...
@section('column_data')
  ...
  @if( isset($items) )
    ...
    <tr>
      ...
      <td>{{ $item->id }}</td>
      <td>
        <a href="{{ url('/posting-detail?id='.$item->id) }}">
        {{ $item->subject }}
        </a>
      </td>
      <td>{{ $item->expo_yn }}</td>
...
<script>
  ...
  $(function(){
    ...
    function Search(page)
    {
      ...
      $(location).attr('href', "{{ url('/posting-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/posting-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/posting/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
