      ...
      <table cellpadding="0" cellspacing="0" style="width:100%;font-family:'Malgun Gothic', Dotum,'돋움',Helvetica,'Apple SD Gothic Neo',sans-serif">
        ...
        <tr>
          <td style="font-size:16px;color:#000;line-height:27px;">
            @if ($type=='01')
              @if($lang=='ko')
                제보하신 내용이 문의하기 [{{ $category }}]으로 접수되었습니다.<br>
                문의하신 내용에 빠르게 답변 드리겠습니다.
              @else
                Your report has been changed to [{{ $category }}] inquiry.
                Your inquiry will be addressed shortly.
              @endif
            @else
              @if($lang=='ko')
                문의하신 내용이 제보하기 [{{ $category }}]으로 접수되었습니다.<br>
                제보하신 내용에 대해 빠르게 답변 드리겠습니다
              @else
                Your inquiry has been changed to [{{ $category }}] report.
                Your report will be addressed shortly.
              @endif
            @endif
          </td>
        </tr>
