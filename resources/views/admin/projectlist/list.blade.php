{{-- 
  // 페이지명 : 프로젝트 실적 관리 목록
  // 설명 : 프로젝트 실적 목록을 페이지 단위로 표시 한다.
--}}

{{-- 1) 보드 리스트형식의 템플릿 --}}
@extends('layouts.board_list', [
      'title' => '프로젝트 관리',
      'subject' => '프로젝트 실적 관리 목록',   
      'lang' => 'ko',
      'tab_no' => 0,
      'menu_no' => 1
  ])
...
@section('column_data')
  ...
  @if( isset($projects) )
    ...
    <tr>
      ...
      <td>
        <a href="{{ url('/projectlist-detail?id='.$data->id) }}">
        {{ $data->name_ko }}
        </a>
      </td>
...
<script>
  ...
  $(function(){
    ...
    function Search(page)
    {
      ...
      $(location).attr('href', "{{ url('/projectlist-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/projectlist-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        ...
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/projectlist/deletes') }}", arrId);
          }
        })
        .catch(()=>{
          // console.log('cancel...');
