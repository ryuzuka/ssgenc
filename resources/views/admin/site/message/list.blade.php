{{-- 
  // 페이지명 : 메인 키비주얼/메세지 관리 목록
  // 설명 : 메인 키비주얼/메세지 관리 목록을 페이지 단위로 표시 한다.
--}}
...
@section('column_data')
  ...
  @if( isset($items) )
    ...
    <tr>
      ...
      <td>{{ $item->msg_acc }}</td>
      <td>
        <a href="{{ url('/message-detail?id='.$item->id) }}">
        {{ ($lang=='ko')?$item->key_msg_ko:$item->key_msg_en }}
        </a>
      </td>
      <td>{{ $item->expo_yn }}</td>
...
<script>
  ...
  $(function(){
    ...
    function Search(page)
    {
      ...
      $(location).attr('href', "{{ url('/message-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/message-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/message/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
