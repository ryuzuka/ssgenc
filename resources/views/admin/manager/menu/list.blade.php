    ...
    <td>
      <div class="select-input">
        ...
        @include('includes.select', [
            'text' => '검색',
            'id_name' => 'item_find_key',
            'items' => json_decode(json_encode([
                  ['code_id'=>'00', 'code_nm'=>'전체'],
                  ['code_id'=>'01', 'code_nm'=>'카테고리'],
                  ['code_id'=>'02', 'code_nm'=>'메뉴명']
                ])),
            ])
...
@section('column_data')
  ...
  @if( isset($items) )
    ...
    <tr>
      ...
      <td>{{ $item->category }}</td>
      <td>
        <a href="{{ url('/menu-detail?id='.$item->menu_id) }}">
        {{ $item->menu_nm }}
        </a>
      </td>
      <td>{{ $item->useflag }}</td>
  ...
  $(function(){
    ...
    function Search(page)
    {
      var params = {
        ...
        from: $("#from").val(),
        to: $("#to").val(),
        item_find_key: $("#item_find_key").val(),
        searchText: $("#searchText").val()
      };      

      // console.log('params ==> ' + JSON.stringify(params));

      $(location).attr('href', "{{ url('/menu-list?') }}"+jQuery.param(params) );
    }
    ...
    function Add(id)
    {
      ...
      $(location).attr('href', "{{ url('/menu-detail?') }}"+jQuery.param(params) );
    }
      ...
      if($(this).is(".btn-primary") === true)
      {
        Vue.confirm('선택한 항목을 삭제하시겠습니까?').then((val)=>{
          if (val == true)
          {
            ...
            Delete("{{ url('api/menu/deletes') }}", arrId);
          }
        })
        .catch(()=>{
        });
